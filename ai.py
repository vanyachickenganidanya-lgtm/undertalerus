...
# 1. НАСТРОЙКА КЛИЕНТА (OpenRouter + Gemini 3.5 Flash)
client = OpenAI(
    base_url="https://openrouter.ai/api/v1",
    api_key="ТВОЙ_OPENROUTER_API_KEY"  # <-- Сюда свой ключ
)
MODEL_NAME = "google/gemini-3.5-flash"
extra_headers = {"HTTP-Referer": "http://localhost", "X-Title": "Gemini+ OS Agent"}
...
def write_file(path, content):
    ...
    try:
        with open(path, "a", encoding="utf-8") as f:  # "a" чтобы дописывать, или "w" чтобы перезаписывать
            f.write(content + "\n")
...
def run_cmd(command):
    ...
    try:
        # shell=True позволяет юзать pacman, g++, cd и любые системные утилиты
...
# Описание инструментов для Gemini 3.5 Flash
tools_definition = [
